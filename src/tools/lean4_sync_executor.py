...
class Lean4SyncExecutor:
    ...
    def __init__(self, 
        ...
        self.process_interface : ProcessInterface = None
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        assert self.process_interface is not None, "ProcessInterface is not initialized"
    ...
    def _check_if_thm_read(self, full_stmt: str) -> bool:
        ...
        try:
            with open(temp_file_full_path, "w") as f:
                f.write(full_stmt)
            self.process_interface.send_command({"path": temp_file_full_path})
            timeout_in_secs = self.timeout_in_sec * 4
            response = self.process_interface.read_response(timeout_in_secs)
    ...
    def _run_stmt_on_lean_server(self, idx : int, stmt: str, theorem_started: bool = False):
        ...
        while not cmd_was_executed:
            ...
            if env_idx is not None:
                cmd["env"] = env_idx
            self.process_interface.send_command(cmd)
            timed_out = False
            try:
                timed_out_in_secs = self.timeout_in_sec
                if use_file:
                    timed_out_in_secs *= 4 # File can be big and take time
                response = self.process_interface.read_response(timed_out_in_secs)
                relevant_messages = []
                if 'messages' in response and (use_file or ('proofState' not in response and 'sorries' not in response)):
                    ...
                    if not self.process_interface.process_is_running():
    ...
    def _skip_to_theorem(self, theorem: str):
        ...
        while not found_theorem and not self.execution_complete:
            ...
            self.current_stmt = stmt
            self.line_num += 1
            #Print(stmt, self._stmt_has_lemma(stmt))
